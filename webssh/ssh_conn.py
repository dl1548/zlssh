...
from webssh.ssh_tools import get_key_obj
...
class SSH:
    ...
    def connect(self, host, user, password=None, ssh_key=None, port=22, timeout=30,
                term='xterm', pty_width=80, pty_height=24,filename='',connect_time=''):
    ...
    def websocket_to_django(self):
        try:
            ...
            message = json.dumps(self.message)
            self.websocker.send(message)
            record_log(type='iodata', data=data,connect_time=self.connect_time,filename=self.filename)
